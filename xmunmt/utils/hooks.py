...
class EvaluationHook(tf.train.SessionRunHook):
    ...
    def __init__(self, eval_fn, eval_input_fn, eval_decode_fn, base_dir,
        ...
        Args:
            eval_fn: A function with signature (feature)
            eval_input_fn: A function with signature ()
            eval_decode_fn: A function with signature (inputs)
            base_dir: A string. Base directory for the checkpoint files.
            session_config: An instance of tf.ConfigProto
            max_to_keep: An integer. The maximum of checkpoints to save
            eval_secs: An integer, eval every N secs.
            eval_steps: An integer, eval every N steps.
