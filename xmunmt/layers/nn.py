...
def maxout(inputs, output_size, maxpart=2, use_bias=True, concat=True,
    ...
    """ Maxout layer
    Args:
        inputs: see the corresponding description of ``linear''
        output_size: see the corresponding description of ``linear''
        maxpart: an integer, the default value is 2
        use_bias: a boolean value indicate whether to use bias term
        scope: the scope of this layer, the default value is ``maxout''

    Returns:
        a Tensor with shape [batch, output_size]

    Raises:
        RuntimeError: see the corresponding description of ``linear''
    """
